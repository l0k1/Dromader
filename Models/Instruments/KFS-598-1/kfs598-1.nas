...
var standby_freq = props.globals.getNode("/instrumentation/comm[1]/frequencies/standby-mhz");
var active_freq = props.globals.getNode("/instrumentation/comm[1]/frequencies/selected-mhz");
var volume = props.globals.initNode("/instrumentation/comm[1]/volume",0.0,"DOUBLE",1);
var pwrbtn = props.globals.getNode("/instrumentation/comm[1]/power-btn");
...
var change_freq = func(amnt) {
  if (mode == STANDBY) {
    standby_freq.setValue(math.periodic(118,136,standby_freq.getValue() + amnt));
    update_bottom(sprintf("%.2f",standby_freq.getValue()));
  } elsif (mode == ACTIVE) {
    active_freq.setValue(math.periodic(118,136,active_freq.getValue() + amnt));
    update_top(sprintf("%.2f",active_freq.getValue()));
  } elsif (mode == CHANNEL) {
    ...
    if (program_mode == CHANNEL) {
      ...
      if (channels[cur_channel] == 0) {
        if (amnt > 0) {
          channels[cur_channel] = 118;
        } else {
          channels[cur_channel] = 136;
        }
      } else {
        channels[cur_channel] = channels[cur_channel] + amnt;
        if (channels[cur_channel] < 118 or channels[cur_channel] > 136) {
...
var LCDKFS598 = {
  ...
  canvas_settings: {
    "name": "LCD_KFS598",   # The name is optional but allow for easier identification
  ...
  new: func(placement)
  {
    var m = {
      parents: [LCDKFS598],
      LCD_KFS598: canvas.new(LCDKFS598.canvas_settings)
    };

    m.LCD_KFS598.addPlacement(placement);
    m.LCD_KFS598.setColorBackground(0,0,0,0);
    m.displays = m.LCD_KFS598.createGroup();
...
var init = setlistener("/sim/signals/fdm-initialized", func() {
  ...
  KFS_LCD_DISPLAY = LCDKFS598.new({"node": "kfs598LCD-1"});
  volume_knob(-0.05);
  update_bottom(sprintf("%.2f",standby_freq.getValue()));
  update_top(sprintf("%.2f",active_freq.getValue()));
});
